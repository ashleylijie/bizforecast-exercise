...
2. 达成利润目标10万元的概率`r mean(dat$profit > 100000)`，亏损风险`r mean(dat$profit < 0)`。
...
4. 达成利润目标10万元的概率`r mean(fit_dat$profit > 100000)`，亏损风险`r mean(fit_dat$profit < 0)`。
...
bind_cols(fit_dat, fix_dat) %>% 
    ggplot() +
    geom_density(aes(x = profit), fill = "lightgray", alpha = 0.3) +
    geom_density(aes(x = profit1), fill = "lightblue", alpha = 0.3)
```

5. 达成利润目标10万元的概率`r mean(fix_dat$profit > 100000)`，亏损风险`r mean(fix_dat$profit < 0)`。