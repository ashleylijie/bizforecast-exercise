...
old <- theme_tufte() + theme(text = element_text(family = "Noto Sans CJK SC", size = 9))
theme_set(old)


set.seed(1212)
cal_profit <- function(n){
    
    profit_margin <- runif(n, min = 350, max = 400)
    leads_n <- runif(n, min = 3000, max = 4000)
    conversion_rate <- rnorm(n, mean = 0.04, sd = 0.005)
    
    revenue <- profit_margin * leads_n * conversion_rate * 30.4
    
    fixed_overheads <- 20000
    cost_per_lead <- runif(n, min = 8, max = 10)
    leads_cost <- leads_n * cost_per_lead
    
    expenses <- (fixed_overheads + leads_cost) * 30.4
    
    res <- data.frame(revenue,
                      expenses,
                      profit = revenue - expenses)
}

dat <- cal_profit(10000)

dat %>% 
    ggplot(aes(x = profit)) +
    geom_density() +
    geom_vline(xintercept = quantile(dat$profit, c(0.025, 0.975)), color = "darkred", lty = 2) +
    annotate("text", label = round(quantile(dat$profit, c(0.025, 0.975))), 
             x = quantile(dat$profit, c(0.025, 0.975)), 
             y = 0, colour = "red")

```

2. 达成利润目标 10 万元的概率 `r prettyNum(mean(dat$profit > 100000) * 100)` %，亏损风险 `r prettyNum(mean(dat$profit < 0) * 100)` %。
...
```{r}
raw <- read_csv("Data/cpl_and_conversion.csv")
set.seed(1212)
lm.fit <- lm(cost_per_lead ~ conversion_rate, raw)

fit_profit <- function(n){
    ...
    fixed_overheads <- 20000
    cost_per_lead <- predict(lm.fit, data.frame(conversion_rate))
    leads_cost <- leads_n * cost_per_lead
...
4. 达成利润目标 10 万元的概率 `r prettyNum(mean(fit_dat$profit > 100000) * 100)` %，亏损风险 `r prettyNum(mean(fit_dat$profit < 0) * 100)` %。

```{r}
set.seed(1212)
fix_profit <- function(n){
    
    profit_margin <- runif(n, min = 350, max = 400)
    leads_n <- runif(n, min = 3000, max = 4000) * runif(n, min = 1.15, max = 1.25)
    conversion_rate <- rnorm(n, mean = 0.04, sd = 0.005)
    
    revenue <- profit_margin * leads_n * conversion_rate * 30.4
    
    fixed_overheads <- 20000 * runif(n, min = 1.25, max = 1.3)
    cost_per_lead <- predict(lm.fit, data.frame(conversion_rate))
    leads_cost <- leads_n * cost_per_lead
...
fix_dat <- fix_profit(10000)

```

5. 达成利润目标 10 万元的概率 `r prettyNum(mean(fix_dat$profit > 100000) * 100)` %，亏损风险 `r prettyNum(mean(fix_dat$profit < 0) * 100)` %。

```{r}
bind_rows(fit_dat %>% mutate(type = "原始成本"), 
          fix_dat %>% mutate(type = "增加成本")) %>% 
    ggplot(aes(x = profit, fill = type, alpha = 0.1)) +
    geom_density() +
    guides(alpha = FALSE)
```